...
class Mezo(object):
    ...
    def rajzol(self):
        pygame.draw.rect(ablak, PIROS, (self.x, self.y, self.szelesseg, self.magassag), 0)    #téglalapot rajzol: az ablakra,szín, méretezés(honnan,mekkora), szegély
        szoveg = font.render(str(self.szam), True, FEHER)   #a saját számát kiírja (true-->lekerekített számok)
        szövegdoboz = szoveg.get_rect(center=((2*self.x+self.szelesseg)/2, (2*self.y+self.magassag)/2))  #középre rakja a szöveget a négyzeten belül
...
nyero_allas = [[100, 100, 1], [200, 100, 2], [300, 100, 3], [400, 100, 4], [100, 200, 5], [200, 200, 6], [300, 200, 7], [400, 200, 8], [100, 300, 9], [200, 300, 10], [300, 300, 11], [400, 300, 12], [100, 400, 13], [200, 400, 14], [300, 400, 15]]
    ...
    while fut:
        for event in pygame.event.get():
            ...
            if event.type == QUIT:
                ...
                if 0 in xy_tavolsag and (100 in xy_tavolsag or -100 in xy_tavolsag):    #megnézem, hogy az üres melletti mezőre kattintottak-e
                    for mezo in allas:
